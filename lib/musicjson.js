...
var attrkey = '$', charkey = '_', orderkey = '%', namekey = '#name';
...
Parser.prototype.close = function(node) {
  var obj = this.stack.pop(), name = obj[namekey], last;

  delete obj[namekey];
  last = this.stack[this.stack.length - 1];

  if (obj[charkey].trim().length === 0) { // No text content
    delete obj[charkey];
  ...
  if (this.stack.length > 0) {
    if (name in last) {
      last[name] = util.isArray(last[name]) ? last[name] : [last[name]];
      last[name].push(obj);
    } else {
      last[name] = obj;
    }
...
function toXML(root, el, nodeName) {
  var element, i, attr, type = typeof el;
    ...
    switch (i) {
      ...
      default:
        if (util.isArray(el[i])) {
          el[i].forEach(function(child) { toXML(element, child, i); });
        } else {
